...
def _make_key(detail_link):
    return _normalize(detail_link)
...
def save_to_google_sheets(results, store_label):
    ...
    try:
        ...
        for row in data_rows:
            ...
            existing_keys.add(_make_key(detail_link))

        print(f"기존 시트 데이터: {len(existing_keys)}개 (상세링크 기준)")

        new_values = []
        skipped = 0
        for item in results:
            key = _make_key(item.get("상품상세페이지링크", ""))
